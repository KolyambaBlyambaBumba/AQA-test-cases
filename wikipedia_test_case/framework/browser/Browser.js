...
class Browser {
   ...
   * @param {Function} condition - The condition function returning true or false
   * @param {number} timeout - Maximum wait time (default 30000 ms)
   * @param {string} timeoutMsg - Message if timeout exceeds
   */
  async waitFor(condition, timeout = 30000, timeoutMsg = "Condition not met in time") {
    Logger.info(`Waiting for condition`);
    return this.#getBrowser().waitUntil(condition, { timeout, timeoutMsg });
  }
};
